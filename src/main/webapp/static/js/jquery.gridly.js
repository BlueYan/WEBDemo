...
(function () {
    ...
    Draggable = (function () {
        ...
        Draggable.prototype.pick = function (target, event, pos, str) {
            //console.log("pos = " + pos + " str = " + str + " event = " + event + " target = " + target);
            var ref;
            return (ref = this.callbacks) != null ? typeof ref.pick === "function" ? ref.pick(target, event, pos, str) : void 0 : void 0;
        };
        ...
        Draggable.prototype.ended = function (event) {
            ...
            //console.log('left = ' + this.$target.position().left + ' top = ' + this.$target.position().top);
            var left = this.$target.position().left;
            var top = this.$target.position().top;
            //console.log("id = " + this.$target.context.id)
            //-3 -212

            if ( (left >= -1 && left <= 75) && ( top >= -310 && top <= -100) ) {
                // 西方位玩家 pos = 0
                this.pick(this.$target, event, 0, this.$target.context.name);
            } else if ( (left >= 370 && left <= 550) && ( top >= -345 && top <= -280) ) {
                // 南方位玩家 pos = 3
                this.pick(this.$target, event, 3, this.$target.context.name);
            } else if ( (left >= 860 && left <= 960) && ( top >= -310 && top <= -100) ) {
                //东方位玩家 pos = 2
                this.pick(this.$target, event, 2, this.$target.context.name);
            } else if ( (left >= 370 && left <= 550) && ( top >= -170 && top <= -90) ) {
                //北方位玩家 pos = 1;
                this.pick(this.$target, event, 1, this.$target.context.name);
            }
            this.bind('off');
    ...
    Gridly = (function () {
        ...
        function Gridly($el, settings) {
            ...
            this.dragged = bind(this.dragged, this);
            this.$el = $el;
        ...
        Gridly.prototype.dragged = function (target, event, pos, str) {
            return this.settings.cb(target, event, pos, str); //回调到html页面上的js方法
        };
